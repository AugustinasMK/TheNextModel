...
class SentiGLV2Definition(object):
...
class SentiGLV2(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        if self.is_train:
            ...
            positives = self.label_to_images[anchor_label]
            positives.remove(name)
            positive = random.choice(positives)
            positive_full_name = osp.join('/'.join(self.images[0][0].split('/')[:-1]), positive)
            positive_img = Image.open(positive_full_name)
